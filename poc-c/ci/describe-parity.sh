...
#
#   poc-c/ci/describe-parity.sh [config.toml ...]
#
...
# poc-c/build/tedge-dot and a Rust binary (target/{debug,release}/tedge-dot, or
# $RUST_BIN); the Rust one is built with cargo when neither exists.
#
...
c_bin="$repo/poc-c/build/tedge-dot"
rust_bin=${RUST_BIN:-}

if [ ! -x "$c_bin" ]; then
    echo "FAIL: $c_bin not built (cmake -B poc-c/build -S poc-c && cmake --build poc-c/build)" >&2
    exit 2
fi
if [ -z "$rust_bin" ]; then
    for candidate in "$repo/target/debug/tedge-dot" "$repo/target/release/tedge-dot"; do
        if [ -x "$candidate" ]; then
            rust_bin=$candidate
            break
        fi
...
if [ -z "$rust_bin" ]; then
    echo "== building the Rust binary (no target/{debug,release}/tedge-dot found)"
    (cd "$repo" && cargo build --quiet)
    rust_bin="$repo/target/debug/tedge-dot"
fi
