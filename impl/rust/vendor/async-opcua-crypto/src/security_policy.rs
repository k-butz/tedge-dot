...
impl SecurityPolicy {
    ...
    pub fn random_nonce(&self) -> ByteString {
        // tedge-dot patch (see vendor/async-opcua-crypto/TEDGE-DOT-PATCH.md): the session
        // nonce returned in ActivateSessionResponse must be at least 32 bytes even on a
        // None-policy channel — every other stack sends one, and open62541 clients
        // (>= 1.4) refuse to activate a session otherwise (BadSecurityChecksFailed).
        // Upstream returns ByteString::null() for None.
        match self {
            SecurityPolicy::None => random::byte_string(32),
