...
`ot-command-forward` uses the command's `service` field, else `tedge-dot-<protocol>` — the service
name of the packaged configurations. With several configurations of one protocol, name the
service: `c8y-fieldbus-import` sets it from `FIELDBUS_SERVICE`.
